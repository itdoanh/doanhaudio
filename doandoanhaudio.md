...
## 2. MODULE 1 (TREBLE): BLIESMA T34A-4 - "ULTRA-LINEAR AEROSPACE BTL"
**Tính chất củ loa:** Độ nhạy 97dB, Le = 3.8 Ohms phẳng, Màng nhôm-magie cứng rung động (Break-up) ở >35kHz.
**Mục tiêu:** Méo hài < 0.001% ở 35kHz, nền âm tĩnh lặng tuyệt đối (Zero Hiss). Áp dụng triệt để triết lý "Phân rã nhiệm vụ" (Task Segregation) bằng các linh kiện rời (Discrete) chuyên biệt, ép lõi Silicon TPA3255 hoạt động ở trạng thái lý tưởng hóa tuyệt đối trong môi trường kiến trúc vi ba (RF-Microwave PCB).

### 2.1. Phân rã Nguồn điện chuyên biệt (Triple-Isolated Power Supply)
*   **PVDD (Nguồn động lực 48V):** Cấp nguồn riêng cho tầng xả dòng TPA3255 qua mạch **Active Ripple Eater (Discrete Capacitance Multiplier)** dùng MOSFET. Triệt tiêu hoàn toàn Ripple sụt áp từ dàn Sub, kết hợp dải tụ gốm MLCC siêu thấp ESL hàn trực tiếp tại chân IC để bóp chết sụt áp Nano-giây.
*   **GVDD & VDD (Nguồn lái cực cổng 12V - Gate Drive):** Cắt đứt hoàn toàn liên kết với buck-converter nội bộ. Sử dụng module biến áp riêng và cụm **Dual LT3045 LDO (Ultra-low noise 0.8 µVrms)** để cấp nguồn 12V độc lập. Nguồn lái cổng phẳng tuyệt đối này đảm bảo Mosfet đóng cắt với sườn xung sắc lẹm, tàn bạo nhất, miễn nhiễm với biến động của dòng ra loa.
*   **AVDD (Nguồn Logic 3.3V):** Lọc qua một tầng LT3045 riêng biệt, bảo vệ lõi so sánh (Comparator) khỏi nhiễu Digital nội sinh.

### 2.2. Phân rã Tầng xử lý Tín hiệu (Instrumentation Discrete Architecture)
Thay thế IC tích hợp bằng Kiến trúc 2 tầng Op-Amp rời (Discrete Op-Amp) nhằm tối ưu hóa 100% từng nhiệm vụ, loại bỏ nhiễu chéo tương hỗ:
1.  **Tầng Buffer cách ly (Outside PFFB Loop):** Sử dụng Discrete Op-Amp nền tảng JFET (vd: Sparkos Labs SS3602). Nhiệm vụ duy nhất: Cách ly trở kháng tuyệt đối với tín hiệu từ DSP, loại bỏ nhiễu đồng pha (CMRR cực đại). Tầng này nằm hoàn toàn bên ngoài vòng lặp PFFB để bảo toàn Phase Margin.
2.  **Tầng Error Amplifier (Inside PFFB Loop):** Sử dụng Discrete Op-Amp siêu tốc độ (Slew rate cao). Nhiệm vụ duy nhất: Đóng vai trò là trạm thu (Summing Node), so sánh tín hiệu sạch từ Buffer với tín hiệu PFFB dội về từ cọc loa, bơm dòng sửa lỗi trực tiếp vào TPA3255. Toàn bộ module Op-Amp rời được phân cực Pure Class-A và cấp nguồn bởi LT3094/LT3045.

### 2.3. Cấu hình Công suất & Xuất âm (Output Stage)
*   **IC Khuếch đại:** Texas Instruments TPA3255 (BTL Direct-Coupled). Ép xung băm xung (Fsw) đồng bộ Master/Slave Clock 600kHz.
*   **Pseudo-GaN SiC Clamping:** Kẹp 8x Diode Schottky Silicon Carbide (SiC) siêu tốc (Qrr=0) từ chân Output xuống GND và PVDD. Mô phỏng sườn xung amply GaN, triệt tiêu tận gốc lực dội ngược sinh méo hài bậc lẻ (H3, H5).
*   **Mạng PFFB Vishay Z-Foil Cực Đoan:** Sử dụng 100% điện trở **Vishay Naked Z-Foil (sai số 0.005%, TCR 0.05ppm/°C)** cho mạng hồi tiếp. Đảm bảo độ cân bằng vi sai hoàn hảo nhất về mặt toán học.
*   **Bộ lọc LC (Load-Invariant 150kHz):** Cuộn cảm 5.0 µH (Lõi không khí Air-Core, dây Litz) + Tụ WIMA FKP2 0.22 µF / 100V. 
*   **RF Termination Zobel chuyên biệt:** Bỏ điện trở/tụ thường. Nâng cấp lên điện trở **Caddock Thick-Film TO-220 (Non-Inductive)** kết hợp tụ **Silver Mica (Mica Bạc)**. Nhiệm vụ duy nhất: Hấp thụ 100% sóng siêu cao tần thành nhiệt lượng mà không sinh ra từ trường ký sinh, bảo vệ củ BlieSMa khỏi cộng hưởng RF.
*   **PCB 6-Layer Rogers RO4350B:** Kiến trúc mạch in 6 lớp dùng vật liệu suy hao điện môi thấp (cấp độ 5G/Radar). Trace vi sai hồi tiếp PFFB chạy qua Via ngầm (Buried Vias) nằm ở lớp nội vi, kẹp giữa các Ground Plane khổng lồ để cách ly bức xạ 4000W.

---

## 3. MODULE 2 (MIDRANGE): SCAN-SPEAK 10F/4424G00 - "DISCRETE HYBRID & ZERO-INERTIA BTL"
**Tính chất củ loa:** Màng siêu nhẹ (Mms = 2.8g), Le = 0.1mH, 4 Ohms. Tốc độ chớp nhoáng, nhạy cảm với méo từ trễ và giao điểm.
**Mục tiêu:** Trễ pha 0 độ (300Hz - 3kHz). Trung âm mang linh hồn analog của amply Class-A thuần túy, lồi 3D mọng nước, tái tạo đến từng vi chi tiết hơi thở nhờ sự phân rã nguồn điện và kiến trúc tín hiệu siêu tinh vi.

### 3.1. Phân rã Nguồn điện & Tầng xử lý Tín hiệu (Aerospace Grade)
*   **Cấu trúc Nguồn Tam lập (Triple-Isolated):** Tương tự dải Treble, Module Mid tách biệt hoàn toàn 3 khối nguồn: PVDD (Nuốt nhiễu Active Ripple Eater), GVDD 12V (LT3045 LDO lái cổng), và AVDD 3.3V. Đảm bảo sức mạnh dòng điện đập vào củ loa Scan-Speak là khối năng lượng sạch và bạo lực nhất, không bị dính tạp âm từ dao động điện áp.
*   **Instrumentation Discrete Architecture:** Tầng tiền khuếch đại phân rã làm 2 khối rời (Buffer JFET Class-A và Error-Amp Summing). Nhờ cấu hình Pure Class-A phân cực cao của các Discrete Op-Amp, dải trung âm bảo tồn nguyên vẹn các hài âm bậc chẵn (H2) tự nhiên, tái tạo giọng Vocal analog, ấm áp và lồi hẳn lên phía trước.

### 3.2. Cấu hình Công suất & Xử lý Âm học (Output Stage)
*   **IC Khuếch đại:** Texas Instruments TPA3255 (Chế độ BTL). Tích hợp SiC Clamping Diode mô phỏng sườn xung GaN, làm sạch mọi gai góc của dải mid.
*   **Bộ lọc LC (Vật liệu Ultra Hi-End 50kHz):** Nâng cấp lên **Cuộn cảm Lá Đồng lõi không khí (Copper Foil Air-Core, vd: Mundorf CFC)** 7.0 µH + **Tụ màng Bạc/Đồng ngâm dầu (Silver/Gold Oil Capacitor)** 1.0 µF. Cấu hình xa xỉ bậc nhất thế giới, bóp chết 100% méo từ trễ (Magnetic Hysteresis), trả lại sân khấu trung âm Holographic 3D siêu thực.
*   **Z-Foil On-Board Kelvin Sense PFFB:** Điểm lấy mẫu hồi tiếp tại trạm xuất âm on-board, chạy vi sai bọc mass (Differential Stripline) trên PCB 6 Lớp. Sử dụng điện trở **Vishay Naked Z-Foil (0.005%)** siêu chính xác. Sự hoàn hảo toán học này khóa cứng thông số amply, bù trừ cực độ mọi biến thiên trở kháng cơ học (Back-EMF) của màng loa 10F bất kể nhiệt độ hệ thống thay đổi.
*   **RF Termination Zobel:** Zobel dập nhiễu cao tần (4.7Ω + 0.1µF) sử dụng trở Caddock Non-Inductive và tụ Silver Mica, loại trừ rủi ro tự cảm phát sinh ở dải siêu âm.
---
